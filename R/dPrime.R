...
#' To do this, the percentage values are converted to z-values with the help of inverse cumulative distribution function (CDF)
...
dPrime <- function(oldRating, ratings, oldNew, oldNewLevels = c(0,1)){
  
  oldStimuli <- ratings[which(oldNew == oldNewLevels[2])]
  hit <- 0
  for(i in 1:length(oldRating)){
    hit <- hit + (sum(oldStimuli == oldRating[i])/length(oldStimuli))
  }
  return(qnorm(hit) - qnorm(1 - hit))
}